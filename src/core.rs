...
use crate::types::{ScanOptions, ScanResult, ScanConfigBuilder, config_utils};
...
impl MCPScannerCore {
    ...
    fn parse_scan_options(&self, request: &ScanRequest) -> Result<ScanOptions> {
        ...
        let mut builder = ScanConfigBuilder::new()
            .timeout(request.timeout.unwrap_or(scanner_config.scanner.scan_timeout))
            .http_timeout(request.http_timeout.unwrap_or(scanner_config.scanner.http_timeout))
            .detailed(request.detailed.unwrap_or(scanner_config.scanner.detailed))
            .format(request.format.clone().unwrap_or(scanner_config.scanner.format));
